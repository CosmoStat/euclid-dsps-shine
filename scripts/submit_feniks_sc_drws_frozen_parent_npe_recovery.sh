...
EXPORTS="ALL,REPO_DIR=$JOB_REPO_DIR,MINICONDA_PATH=$MINICONDA_PATH,CONDA_ENV=$CONDA_ENV,NPE_ROOT=$NPE_ROOT,CACHE_ROOT=$CACHE_ROOT,BENCHMARK_ENV=$BENCHMARK_ENV,BASELINE_ENV=$BASELINE_ENV,NPE_RECOVERY_COMMIT=$CODE_COMMIT"
...
printf 'export ORIGINAL_ARM_JOB=%q\nexport ARM_JOB=%q\nexport SCRATCH_RECOVERY_JOB=%q\nexport GATE_JOB=%q\nexport SUBMIT_EVALUATION_JOB=%q\nexport ALL_JOBS=%q\nexport NPE_ROOT=%q\nexport NPE_LOG_ROOT=%q\nexport BASELINE_ENV=%q\nexport BENCHMARK_ENV=%q\nexport SOURCE_BENCHMARK_ROOT=%q\nexport RECOVERY_ROOT=%q\nexport JOB_REPO_DIR=%q\nexport CODE_COMMIT=%q\nexport NPE_RECOVERY_COMMIT=%q\n' \
  ...
  "$SOURCE_BENCHMARK_ROOT" "$RECOVERY_ROOT" "$JOB_REPO_DIR" \
  "$CODE_COMMIT" "$CODE_COMMIT" > "$ENV_FILE"
