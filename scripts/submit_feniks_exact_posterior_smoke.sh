...
ROOT_DIR="${ROOT_DIR:-outputs/runs/feniks_exact_posterior_smoke_${STAMP}}"
...
test ! -e "$ROOT_DIR" || {
  echo "[exact-smoke][error] output already exists: $ROOT_DIR"
  exit 2
}
...
prep=$(sbatch --parsable --array=0-1%2 --time=00:30:00 \
  --export="$common_export,MODE=smoke,N_GALAXIES=2" \
...
log="outputs/logs/submit_feniks_exact_smoke_${STAMP}.log"
{
  printf 'root=%q\n' "$ROOT_DIR"
...
echo "requested_upper_bound_h100_hours=8.33 peak_h100=12"
