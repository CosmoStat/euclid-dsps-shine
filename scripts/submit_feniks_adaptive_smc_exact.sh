...
source "${TRAIN_ENV:-outputs/logs/feniks_adaptive_smcwake_latest.env}"
...
COHORT_FILE="${COHORT_FILE:-$MANIFEST_ROOT/exact_cohort.csv}"
...
test -e "$TRAIN_ROOT/DONE" || {
  echo "[feniks-asmc-exact-submit][error] training is incomplete" >&2; exit 2;
}
for path in "$TRAIN_ROOT/training_receipt.json" "$CONFIG" "$DATASET" \
  "$CHECKPOINT" "${CHECKPOINT}.json" "$FEATURE_STATS" "$COHORT_FILE"; do
  test -s "$path" || {
    echo "[feniks-asmc-exact-submit][error] missing: $path" >&2; exit 2;
  }
done
python scripts/validate_feniks_adaptive_smc_training.py --train "$TRAIN_ROOT"
test ! -e "$EXACT_ROOT" || {
  echo "[feniks-asmc-exact-submit][error] output exists: $EXACT_ROOT" >&2; exit 2;
}
...
[[ "$actual" == "$N_GALAXIES" ]] || {
  echo "[feniks-asmc-exact-submit][error] cohort has $actual rows, expected $N_GALAXIES" >&2
  exit 2
}

export REPO_DIR MINICONDA_PATH CONDA_ENV CONFIG DATASET CHECKPOINT FEATURE_STATS
export MANIFEST_ROOT
...
latest=outputs/logs/feniks_adaptive_smc_exact_latest.env
printf 'export EXACT_JOB=%q\nexport EXACT_FINALIZER_JOB=%q\nexport EXACT_ROOT=%q\nexport N_GALAXIES=%q\nexport CONFIG=%q\nexport DATASET=%q\nexport CHECKPOINT=%q\nexport FEATURE_STATS=%q\nexport MANIFEST_ROOT=%q\n' \
  ...
  "$CONFIG" "$DATASET" "$CHECKPOINT" "$FEATURE_STATS" "$MANIFEST_ROOT" \
  > "$latest"
...
echo "methods=q+q_IS+defensive_IS+adaptive_SMC+MAP+NUTS+truth"
