#!/bin/bash
# Usage: bash scripts/submit_feniks_ratio_convergence.sh SOURCE_FOLLOWUP NEW_ROOT [CONFIG]
set -Eeuo pipefail

SOURCE=$(realpath "${1:?completed ratio-followup root}")
export FENIKS_RATIO_CONVERGENCE_ROOT=$(realpath -m "${2:?new convergence root}")
CONFIG=$(realpath "${3:-configs/experiments/feniks_ratio_convergence.yaml}")
REPO=$(pwd -P)

test ! -e "$FENIKS_RATIO_CONVERGENCE_ROOT"
test ! -e "$FENIKS_RATIO_CONVERGENCE_ROOT.code.tar"
command -v sbatch >/dev/null
mkdir -p "$(dirname "$FENIKS_RATIO_CONVERGENCE_ROOT")"

tar --exclude='__pycache__' --exclude='*.pyc' \
  -cf "$FENIKS_RATIO_CONVERGENCE_ROOT.code.tar" \
  euclid_dsps scripts configs pyproject.toml
tar --dereference -rf "$FENIKS_RATIO_CONVERGENCE_ROOT.code.tar" filters
DIGEST=$(sha256sum "$FENIKS_RATIO_CONVERGENCE_ROOT.code.tar" | cut -d' ' -f1)
...
tar -xf "$FENIKS_RATIO_CONVERGENCE_ROOT.code.tar" \
  -C "$FENIKS_RATIO_CONVERGENCE_CODE"
[[ -e "$FENIKS_RATIO_CONVERGENCE_CODE/Data" ]] || \
  ln -s "$REPO/Data" "$FENIKS_RATIO_CONVERGENCE_CODE/Data"

cd "$FENIKS_RATIO_CONVERGENCE_CODE"
JAX_PLATFORMS=cpu EUCLID_DSPS_JAX_PLATFORMS=cpu \
  EUCLID_DSPS_REQUIRE_GPU=0 JAX_ENABLE_X64=true \
  python -m scripts.feniks_ratio_convergence prepare \
    --source "$SOURCE" --root "$FENIKS_RATIO_CONVERGENCE_ROOT" \
    --config "$CONFIG"
...
RAW=$(sbatch --parsable --time="$CLASSIFIER_HOURS:00:00" \
  ...
  scripts/feniks_ratio_convergence.slurm)
CLASSIFIER_JOB=${RAW%%;*}
...
echo "Allocation ceiling: $((2 * CLASSIFIER_HOURS + REPORT_HOURS)) H100-hours, not expected runtime."
