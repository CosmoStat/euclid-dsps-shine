...
SPLINE_DIR="Data/diffsky/synthetic/feniks_260617_spline15d_grouped_jaxcosmo_dequant1e3_v1"
CATALOG_DIR="${SPLINE_DIR}/amortized"
PRIOR_RUN="outputs/runs/feniks_spline15d_rqspline_dequant1e3_v1"
AMORTIZED_RUN="outputs/runs/feniks_spline15d_rqspline_dequant1e3_amortized_v1"
INFERENCE_RUN="outputs/runs/feniks_spline15d_rqspline_dequant1e3_inference_v1"
PROJECTION_CONFIG="configs/feniks_spline15d_grouped_jaxcosmo_dequant1e3_v1.yaml"
PRIOR_CONFIG="configs/prior_feniks_spline15d_rqspline_dequant1e3_v1.yaml"
AMORTIZED_CONFIG="configs/amortized_feniks_spline15d_rqspline_dequant1e3_v1_gpu.yaml"

for path in "$SOURCE_DIR/train.parquet" "$SOURCE_DIR/validation.parquet" \
  "$SOURCE_DIR/test.parquet" "$SOURCE_DIR/grouped_split_audit.json"; do
  test -s "$path" || { echo "[submit][error] missing source: $path"; exit 2; }
done
for path in "$SPLINE_DIR" "$PRIOR_RUN" "$AMORTIZED_RUN" "$INFERENCE_RUN"; do
  test ! -e "$path" || {
    echo "[submit][error] refusing existing output: $path"; exit 2;
  }
done
...
prior_job=$(submit --dependency="afterok:${projection_job}" \
  --export="ALL,SPLINE_DATASET_DIR=${SPLINE_DIR},OUT_DIR=${PRIOR_RUN},PRIOR_CONFIG=${PRIOR_CONFIG}" \
  scripts/feniks_spline15d_rqspline_h100.slurm)
amortized_job=$(submit --dependency="afterok:${prior_job}" --gres=gpu:4 \
