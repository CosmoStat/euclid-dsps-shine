#!/usr/bin/env python3
"""Run the bounded exact-SMC q-bootstrap curriculum on FENIKS."""
...
from euclid_dsps.amortized.adaptive_smc_trainer import (
    run_feniks_adaptive_smc_q_curriculum,
)
...
def main() -> None:
    ...
    parser.add_argument("--exact-objects", type=int, default=32)
    parser.add_argument("--extended-max-stages", type=int, default=48)
    parser.add_argument("--distillation-steps", type=int, default=4)
    ...
    receipt = run_feniks_adaptive_smc_q_curriculum(
        config,
        args.out,
