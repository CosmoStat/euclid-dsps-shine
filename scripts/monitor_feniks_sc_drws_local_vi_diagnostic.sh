...
if mode == 'precision_night':
    ...
    if 'qualified_night' in manifest:
        ...
        print('Wake independent-batch diagnostic; evaluation never selects updates' if 'wake_holdout' in manifest else
              'Wake Armijo descent pilot; fixed parent and independent evaluations' if 'wake_backtracking' in manifest else
