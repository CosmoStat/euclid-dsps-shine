...
for task in 0 1 2; do
  case "$task" in
    0) arm=A; previous_job="$VALIDATION_A_JOB" ;;
    1) arm=B; previous_job="$VALIDATION_B_JOB" ;;
    2) arm=C; previous_job="$VALIDATION_C_JOB" ;;
  esac
  for receipt in \
  ...
  if [[ ! -s "$PILOT_ROOT/validation/$arm/VALIDATION_COMPLETE.json" ]]; then
    if [[ -n "$(squeue -h -j "$previous_job" 2>/dev/null)" ]]; then
      echo "[topology-internal-recovery][error] validation $arm is still active as $previous_job; wait for it to finish" >&2
      exit 2
    fi
    MISSING_TASKS+=("$task")
  fi
...
[[ ${#MISSING_TASKS[@]} -gt 0 ]] || {
  echo "[topology-internal-recovery][error] all arm validations are already complete" >&2
  exit 2
}
...
AUTHORIZATION="$PILOT_ROOT/INTERNAL_VALIDATION_RECOVERY_AUTHORIZATION_${RECOVERY_CODE_COMMIT:0:12}.json"
...
python - "$AUTHORIZATION" "$MANIFEST" "$ORIGINAL_CODE_COMMIT" "$RECOVERY_CODE_COMMIT" \
  "$FAILED_VALIDATION_A_JOB" "$FAILED_VALIDATION_B_JOB" "$FAILED_VALIDATION_C_JOB" \
  "$ARRAY_SPEC" <<'PY'
import hashlib
import json
import sys
...
payload = {
    ...
    "replacement_array_tasks": sys.argv[8],
...
VALIDATION_RAW=$(sbatch --parsable --array="$ARRAY_SPEC%3" \
...
for task in "${MISSING_TASKS[@]}"; do
  case "$task" in
    0) VALIDATION_A_JOB="${VALIDATION_JOB}_0" ;;
    1) VALIDATION_B_JOB="${VALIDATION_JOB}_1" ;;
    2) VALIDATION_C_JOB="${VALIDATION_JOB}_2" ;;
  esac
done
...
echo "replacement_array_tasks=$ARRAY_SPEC"
