#!/bin/bash
set -Eeuo pipefail

ROOT=$(realpath "${1:?weighted truth follow-up root}")
ONCE=${2:-}

while true; do
  if [[ -t 1 && "$ONCE" != --once ]]; then printf '\033[H\033[2J'; fi
  date
  if [[ -f "$ROOT/JOBS.env" ]]; then
    source "$ROOT/JOBS.env"
    squeue -j "$ALL_JOBS" -o '%.22i %.12T %.12M %R' || true
    FAILURES=$(sacct -X -n -j "$ALL_JOBS" \
      -s FAILED,TIMEOUT,OUT_OF_MEMORY,CANCELLED \
      --format=JobID,State,ExitCode) || FAILURES='sacct unavailable'
    if [[ -n "${FAILURES//[[:space:]]/}" ]]; then
      printf '\nFAILURES\n%s\n' "$FAILURES"
    fi
  fi
  python - "$ROOT" <<'PY'
import json
import sys
from pathlib import Path

root = Path(sys.argv[1])

def load(path):
    try:
        return json.loads(path.read_text())
    except (FileNotFoundError, json.JSONDecodeError):
        return {}

print("\nARM             FLOW  STATE     EPOCH   BEST NLL   PHYS SW")
for arm in ("resampled", "exact_weighted"):
    for replica in range(2):
        directory = root / arm / f"replica_{replica}"
        final = load(directory / "FINAL.json")
        progress = load(directory / "PROGRESS.json")
        metric = load(directory / "METRIC_PROGRESS.json")
        state = "DONE" if final else ("RUNNING" if progress else "WAITING")
        epoch = final.get("added_epochs", progress.get("epoch", "-"))
        best = progress.get("best_nll")
        sw = final.get("physical_sliced_wasserstein", metric.get("physical_sliced_wasserstein"))
        number = lambda value: f"{value:9.4f}" if isinstance(value, (int, float)) else f"{'-':>9}"
        print(f"{arm:16} F{replica + 1}    {state:8} {str(epoch):>5} {number(best)} {number(sw)}")
report = load(root / "report/FINAL.json")
print("\nReport:", "DONE" if report else "WAITING")
if report:
    print("Conclusion:", report["conclusion"])
    print("Final physical SW:", report["final_physical_sliced_wasserstein"])
PY
  [[ "$ONCE" == --once ]] && break
  echo 'Ctrl-C stops only this watcher. Refresh in 30 seconds.'
  sleep 30
done
