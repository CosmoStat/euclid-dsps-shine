...
final_raw=$(sbatch --parsable --dependency="afterok:${SMC_DIAGNOSTIC_JOB}" \
  --export=ALL,REPO_DIR="$REPO_DIR",MINICONDA_PATH="$MINICONDA_PATH",CONDA_ENV="$CONDA_ENV",OUTPUT_ROOT="$DIAGNOSTIC_ROOT",N_SHARDS="$N_SHARDS",LIMIT="$LIMIT",SMC_VARIANTS_CSV="$SMC_VARIANTS_CSV",SMC_SEEDS_CSV="$SMC_SEEDS_CSV" \
